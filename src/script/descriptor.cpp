...
struct KeyParser {
    ...
    bool KeyCompare(const Key& a, const Key& b) const {
        // Use the signing provider populated during parsing so that
        // hardened-path participants can resolve their pubkeys. An
        // empty provider causes GetPubKey to return nullopt for those
        // keys, leading to false-positive duplicate detection.
        FlatSigningProvider dummy;
        const SigningProvider& sign_provider = m_out ? *m_out : dummy;
        FlatSigningProvider out_a, out_b;
        std::optional<CPubKey> pub_a = m_keys.at(a).at(0)->GetPubKey(0, sign_provider, out_a);
        std::optional<CPubKey> pub_b = m_keys.at(b).at(0)->GetPubKey(0, sign_provider, out_b);
        return pub_a < pub_b;
    }
